"""E7b (2026-08-24): mRI scalar+hist at ep240 fold-wise - the ep120 scalar arm trains
only to 0.858 under fold-wise ranges, below the floor, so the budget is extended.
Original docstring follows.

"""
"""E6+E7: the direct cancellation controls the
...
if __name__ == "__main__":
    ...
    mr = mri_records()
    if SMOKE:
        idx = np.random.RandomState(0).permutation(len(mr))[:200]; mr = [mr[i] for i in idx]
    run("mRI", mr, 10, ("v_sum", "v_hist4", "v_possum_negsum"), results, preds)
    ...
    json.dump(out, open(os.path.join(DOCS, f"sign_controls_mri240{sfx}.json"), "w"), indent=1)
    np.savez_compressed(os.path.join(DOCS, f"sign_controls_mri240{sfx}_preds.npz"), **preds)
